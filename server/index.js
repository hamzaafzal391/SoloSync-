...
app.post('/api/insights', async (req, res) => {
  ...
  function inferCategory(title) {
    ...
    if (/(tuition|school|course|university|college|books)/.test(t))
      return 'Education'
  ...
  try {
    ...
    if (!resp.ok) {
      res.status(resp.status).json({
        error: data?.error?.message || data?.message || resp.statusText || 'Gemini request failed',
        details: data,
      })
      return
    }
    ...
    if (text && !text.trim().endsWith('END')) {
